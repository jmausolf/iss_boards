...
df3a <- dfb %>% 
  #filter(pair_callback_bin != 2) %>% 
  #select(event_match_ep_party_bp_pid2n_mean, board_change_events_list, bp_pid2n_median, new_bm_party, dropped_bm_party) %>%
  select(board_change_events_list, bp_pid2n_median, new_bm_party, dropped_bm_party) %>%
  rename(c("bp_pid2n_median" = "board_party",
         "board_change_events_list" =  "event")) %>%
  filter(event != "NO_CHANGE") %>% 
  filter(new_bm_party != "UNK" & !is.na(new_bm_party)) %>% 
  #mutate(event_match = if_else(em_var  == "YES", 1, 0)) %>% 
  group_by(board_party, event) %>% 
  add_tally( name = "total_events") %>% 
  add_count(new_bm_party, name = "total_in") %>%
  mutate(total_out = NA) %>% 
  #add_count(dropped_bm_party, name = "total_out") %>%
  #mutate(proportion_em = total_event_match / total_events) %>% 
  select(-dropped_bm_party) %>% 
  distinct()
...
df3b <- dfb %>% 
  ...
  rename(c("bp_pid2n_median" = "board_party",
         "board_change_events_list" =  "event")) %>%
  filter(event != "NO_CHANGE") %>% 
  filter(dropped_bm_party != "UNK" & !is.na(dropped_bm_party)) %>% 
  #mutate(event_match = if_else(em_var  == "YES", 1, 0)) %>% 
  group_by(board_party, event) %>% 
  add_tally( name = "total_events") %>% 
  mutate(total_in = NA) %>% 
...
party_types <- c(
  "DEM" = "Democratic Boards",
  "NEU" = "Neutral",
  "REP" = "Republican Boards"
)
...
g <- ggplot(df2_full, aes(event, proportion_em, fill=em_var_party)) +
    ...
    geom_label(aes(x = 0.75, y = 0.85, label = "Incomimg"), 
               ...
               family="Helvetica", 
               size = 5) +
    geom_label(aes(x = 2.5, y = 0.85, label = "Outgoing"), 
               hjust = 0, 
               vjust = 0.5, 
               colour = "#565656", 
               fill = "white", 
...
g
# finalise_plot(g, "", "output/plots/test_incoming_outgoing_bm_by_board_party.png",
...
colors_parties4 = c("#2129B0", "#BF1200", "#2129B0", "#BF1200")
...
df_var_cycle_graph1 <- dfy_full %>% 
    #group_by(event, year) %>% 
    #summarise(meanvar = mean(proportion_em, na.rm = T)) %>%
    filter(em_var == "YES") %>% 
    filter(event != "DROP") %>% 
    ungroup() %>% 
    mutate(board_party = "ALL") %>% 
    mutate(event = factor(event, levels = c("SWAP", "ADD"))) %>% 
    mutate(event_var_party = factor(str_c(event, board_party, sep = '_')))

  


g2 <- ggplot(df_var_cycle_graph1, aes(make_datetime(year), proportion_em)) +
    geom_smooth(color="#3A084A", alpha=0.15, size=0.5, method = "glm") +
    geom_line(aes(color=event_var_party), alpha=0.9) +
    ...
    theme(axis.title = element_text(size = 18)) +
    #xlab("") +
    ylab("Partisan Matching Percentage") +
    labs(title = "Appointments by Board Party and Year",
         caption = "") +
    theme(plot.title = element_text(hjust = 0.5)) +
    ...
    #Color Scale
    scale_color_manual(values=colors_parties4) +
  
    #Add spacing between horz panels
...
g2
finalise_plot(g2, "", "output/plots/party_match_by_year.png",
              footer=FALSE, width_pixels=640, height_pixels=450)
```



```{r boardparty_by_year}

colors_parties6 = c("#2129B0", "#BF1200", "#2129B0", "#BF1200")
...
g1 <- ggplot(df_var_cycle_graph2, aes(make_datetime(year), proportion_em)) +
    geom_smooth(aes(color=event_var_party), alpha=0.15, size=0.5, method = "glm") +
    geom_line(aes(color=event_var_party), alpha=0.9)  +
    facet_grid(event~board_party, labeller = as_labeller(grid_types)) +
    ...
    #Color Scale
    scale_color_manual(values=colors_parties6) +
...
g1
finalise_plot(g1, "", "output/plots/party_match_by_board_party_year.png",
              footer=FALSE, width_pixels=640, height_pixels=450)
...
```{r boardparty_by_year_combined}

df_var_cycle_graph <- bind_rows(df_var_cycle_graph1, df_var_cycle_graph2) %>%
  mutate(board_party = factor(board_party, levels = c("DEM", "REP", "ALL") )) %>% 
  mutate(event_var_party = factor(event_var_party, levels = c("SWAP_DEM", "ADD_DEM", "SWAP_REP", "ADD_REP", "SWAP_ALL", "ADD_ALL")) )




colors_parties6 = c("#2129B0", "#2129B0", "#BF1200", "#BF1200", "#969696", "#969696")
...
g <- ggplot(df_var_cycle_graph, aes(make_datetime(year), proportion_em)) +
    geom_smooth(aes(color=event_var_party), alpha=0.15, size=0.5, method = "glm") +
    geom_line(aes(color=event_var_party), alpha=0.9)  +
    facet_grid(event~board_party, labeller = as_labeller(grid_types)) +
...
g
finalise_plot(g, "", "output/plots/party_match_by_board_party_year_all.png",
              footer=FALSE, width_pixels=640, height_pixels=450)
  
```


```{r boardparty_by_year_combined v2}
...
g
finalise_plot(g, "", "output/plots/party_match_by_board_party_year_all_v2.png",
              footer=FALSE, width_pixels=640, height_pixels=450)
  
```

```{r boardparty_by_cycle_combined v3}

colors_parties4 = c("#2129B0", "#BF1200", "#2129B0", "#BF1200")

dfy <- dfb %>% 
  #filter(pair_callback_bin != 2) %>% 
  select(event_match_ep_pid2ni_med_str_bp_pid2ni_med_mean, board_change_events_list, bp_pid2ni_mean_median, cycle, new_bm_party, dropped_bm_party) %>% 
  rename(c("event_match_ep_pid2ni_med_str_bp_pid2ni_med_mean" = "em_var",
         "bp_pid2ni_mean_median" = "board_party",
         "board_change_events_list" =  "event")) %>%
  filter(event != "NO_CHANGE") %>% 
...
dfy <- dfb %>% 
  ...
  #filter(new_bm_party != "UNK" | is.na(new_bm_party)) %>%
  mutate(event_match = if_else(em_var  == "YES", 1, 0)) %>% 
  group_by(event, cycle, board_party) %>% 
  add_tally( name = "total_events") %>% 
  add_count(event_match, name = "total_event_match") %>% 
  mutate(proportion_em = total_event_match / total_events) %>% 
  select(-event_match) %>% 
...
g <- ggplot(df_var_cycle_graph, aes(make_datetime(cycle), proportion_em)) +
    geom_smooth(aes(color=event_var_party), alpha=0.15, size=0.5, method = "glm") +
    geom_line(aes(color=event_var_party), alpha=0.9)  +
    facet_grid(event~board_party, labeller = as_labeller(grid_types)) +
    ...
    #xlab("") +
    ylab("Partisan Matching Percentage") +
    labs(title = "Appointments by Board Party and Cycle",
         caption = "") +
    theme(plot.title = element_text(hjust = 0.5)) +
...
g
finalise_plot(g, "", "output/plots/party_match_by_board_party_cycle_all.png",
              footer=FALSE, width_pixels=640, height_pixels=450)
